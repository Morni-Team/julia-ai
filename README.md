...
- **Content-Studio komplett entfernt** – das gesamte Content-Modul (Reiter **Schnitt, Planung, Thumbnails** samt 3D-Blender-Render) wurde auf ausdrücklichen Wunsch **vollständig entfernt**. Der „Content"-Menüpunkt ist weg, es wird nichts mehr davon geladen. Julias Kern bleibt unberührt (Bildschirm sehen, sprechen, PC steuern, Minecraft, Updates). Ein frischer, eigenständiger Neuanfang für das Thema ist separat geplant.
- **Content-Studio: echte 3D-Thumbnails aus deinem Skin (Blender)** – der große Wurf: Julia baut jetzt **echte 3D-Minecraft-Thumbnails** wie die großen Kanäle. Sie rendert deinen Skin (über den Minecraft-Namen) als **3D-Figur** in einer Klötzchen-Szene, wählt **Pose, Item und Anordnung passend zum Thema** selbst (z. B. kampfbereit mit Schwert), kann **mehrere Spieler** ins Bild setzen (auch „er greift dich an") und unten gibt es eine **Chatbox**, in der du sagst, was noch geändert werden soll („Schwert größer", „anderer Hintergrund"). Blender wird beim allerersten Mal **einmalig** geladen (~383 MB), danach läuft alles offline auf deinem PC. Im Reiter **Content → Thumbnails** unter „3D-Render".
- **Thumbnails: Skin erscheint jetzt wirklich + echte KI-Hintergründe** – zwei große Sachen: (1) Der **Skin wurde gar nicht angezeigt**, weil der genutzte Render-Dienst tot war – jetzt kommt dein Skin über einen **funktionierenden 3D-Renderer** (NMSR) tatsächlich rein (Minecraft-Namen im Reiter „Kanal" eintragen). (2) Statt eines flachen Farbverlaufs erzeugt Julia jetzt einen **echten KI-Hintergrund** aus deiner Beschreibung (kostenlose Bild-KI, kein Schlüssel) – dein Skin (mit Kontur/Schatten/Glow) und der fette Text kommen als Ebene darüber. Qualität vor Tempo: die KI-Hintergründe dürfen ein, zwei Minuten brauchen. (Recherchiert nach aktuellen High-CTR-Regeln: ein Motiv, Drittel-Regel, 2–3 knallige Farben, Motiv klar vom Hintergrund trennen.)
- **Thumbnails sehen jetzt richtig gut aus (Skin poppt raus)** – der Thumbnail-Bau wurde auf Profi-Niveau gebracht: Dein **Minecraft-Skin** kommt als **cinematische 3D-Pose** rein (aus dem Skin-Render-Dienst, passend zur Kategorie – z. B. „ultimate" bei Minecraft-Gaming) und wird mit **weißer Sticker-Kontur, weichem Schlagschatten und Akzent-Glow** freigestellt, sodass er klar vom Hintergrund abhebt (statt flach draufgeklatscht). Dazu ein kräftigerer Hintergrund (Spotlight + Bokeh-Tiefe + Vignette) und größerer, fetter Text mit dickem Rand und Schatten. (Ein echtes Blender-Render ist in der App nicht drin – die Posen sind aber schon echte 3D-Renders deines Skins.)
- **Thumbnail „Aus Beschreibung" geht jetzt auch im Claude-Abo-Modus** – im Claude-Code-Modus kam „Konzept fehlgeschlagen: Ein Einmal-Aufruf ist im Claude-Code-Modus nicht verfügbar". Jetzt läuft das Konzept-Erstellen (und andere Einmal-Aufrufe) auch über dein **Claude-Abo**. Fürs „Aus Video"-Ansehen braucht es weiterhin einen Anbieter mit Bild-Unterstützung – falls dein Modus keine Bilder kann, sagt Julia das jetzt klar und verweist auf „Aus Beschreibung".
- **Content-Studio: echtes Mini-App-Design + Thumbnails aus einer Beschreibung** – das Content-Modul sieht jetzt aus wie eine **eigene App**: Studio-Kopfzeile mit Kanal-Bild/Badge, Icon-Reiter (✂️ Schnitt · 🗓️ Planung · 🖼️ Thumbnails · ⚙️ Kanal). Neu bei den **Thumbnails**: Du kannst jetzt zwischen **„Aus Video"** (Julia sieht sich das Video an) und **„Aus Beschreibung"** wählen – beschreib einfach dein Thumbnail, und Julia baut daraus ein **cinematisches Bild mit richtigem Hintergrund** (Farbverlauf + Strahlen-Burst + Spotlight hinter deinem Skin + Vignette) und fettem Text. Und es ist jetzt **klar sichtbar**, dass du oben **Gaming oder Reaction** wählen sollst (bestimmt Skin-Pose und Stil).
- **Content ist jetzt eine „App in der App" mit 3 Reitern + Kanal-Leiste** – komplett neu aufgebaut, wie du es wolltest: oben eine **Konto-/Kanal-Leiste** (Bild, Kanalname, Kategorie-Badge – Kanal wechseln wie bei YouTube), darunter **drei Reiter**: **Schnitt** (Schnittaufträge: Rohcut wählen + sagen, wann/wie geschnitten wird – geordnete Warteschlange), **Planung** (was machst du wann, mit Idee und kleinem Skript) und **Thumbnails**. Fürs Thumbnail **sieht sich Julia das Video wirklich an** (mehrere Standbilder gehen ans Bild-Modell), schlägt das beste Standbild + Text/Farben/Aufbau vor, holt aus deinem **Minecraft-Namen** den **Skin** (cinematische Render-Pose) und baut daraus ein fertiges **1280×720-Thumbnail**, das du direkt speicherst. Pro Kanal lässt sich eine **feste Kategorie** einstellen (z. B. MoinMornhart → Minecraft-Gaming), sonst wählst du je Video Reaction/Gaming. (Echte YouTube-Anmeldung mit Live-Statistiken ist der nächste Schritt.)
- **Content: Thumbnails + „Kanal analysieren" geht jetzt sichtbar** – zwei Sachen behoben: (1) Im Analyse-Bereich schaltet der Knopf **„Kanal"** jetzt wirklich um – der Video-Upload verschwindet und das Textfeld bittet klar um deine **Kanal-Infos** (vorher passierte sichtbar nichts). (2) Es gibt einen neuen **Thumbnails**-Bereich: wähle ein Video, und Julia zieht dir mehrere gute **Standbilder (1280×720)** als Ausgangspunkt – die kannst du direkt als Bild speichern. (Text/Logo/dein Skin obendrauf kommen als nächster Schritt.)
- **Update: kein halber Zustand mehr, wenn der Installer nicht startet** – schlug beim Auto-Update der Start des Installers fehl (Datei gesperrt oder ein Prozess hielt sie fest), beendete sich Julia bisher **trotzdem** – das Update lief nie an, und im Log stand nur „gestartet" ohne Grund. Jetzt fängt Julia einen fehlgeschlagenen Installer-Start ab, **bleibt offen** (statt sich in ein halbes Update zu verabschieden) und schreibt die **genaue Ursache** ins Logbuch. (Der endgültige „Datei-gelockt"-Fix braucht noch deine konkrete Log-Zeile; die Dauerlösung ist ein Signatur-Zertifikat.)
- **Content: „Dein YouTube-Kanal" ist jetzt einklappbar** – die Kanal-Karte im Content-Tab wird nicht mehr dauerhaft angezeigt. Sie ist jetzt **eingeklappt** und nur zum **Einrichten** da: einmal aufklappen, Link/Name/Sprache/Ordner eintragen, fertig – danach bleibt der Bereich aufgeräumt.
- **Content-Modul: Schnitt-Engine (Grundlage)** – der Unterbau für automatisches Schneiden ist da und getestet: aus einem Video erkennt Julia Stille und baut einen **Rohschnitt** (Jump-Cut, ohne Adobe), schneidet daraus **Hoch-Clips für TikTok/YouTube Shorts** (9:16) und zieht **Thumbnail-Standbilder** – alles per FFmpeg. Das Content-Profil merkt sich jetzt zusätzlich deinen **Minecraft-Namen** und ein **Thumbnail-Thema** (z. B. Gaming/Minecraft, Reactions). Die Bedien-Oberfläche dazu (Bereiche „Schnitt & Planung" / „Thumbnails") kommt als Nächstes.
- **Content-Modul: neues, sauberes Design + Video-Upload** – der „Content"-Tab sieht jetzt aus wie der Rest von Julia (gleiche Farben/Karten/Buttons, orange Akzente) statt zusammengewürfelt. Neu: Du kannst unter „Analyse" auch direkt ein **Video hochladen** – Julia transkribiert es lokal (Whisper) und gibt dir dann das Creator-Feedback. Der Kanal-Link bleibt optional; Ordner wählst du per Dialog; Assets macht Julia selbst.
- **Minecraft: hängt nicht mehr im Wasser fest** – stand Julia mit einer laufenden Aufgabe im flachen Wasser eingekeilt, kam sie manchmal nicht mehr weg. Jetzt taucht sie in so einem Fall von selbst auf und sucht sich einen neuen Weg heraus.
- **„Hey Julia" wird zuverlässiger erkannt** – die Erkennungs-Schwelle fürs Weckwort war zu streng eingestellt und hat „Hey Julia" oft verschluckt; sie ist jetzt empfindlicher. Zur Erinnerung: Das Weckwort ist **standardmäßig aus** (im Tray/den Einstellungen „Aktivierungswort" einschalten) und braucht die **deutsche Windows-Spracherkennung** – hört sie dich trotzdem nicht, sagt dir Julia jetzt genauer, woran es liegt.
