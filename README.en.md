...
- **Content Studio: real 3D thumbnails from your skin (Blender)** – the big one: Julia now builds **real 3D Minecraft thumbnails** like the big channels. It renders your skin (via your Minecraft name) as a **3D character** in a blocky scene, picks **pose, item and arrangement to fit the topic** by itself (e.g. battle-ready with a sword), can put **several players** in the shot (including "he's attacking you"), and there's a **chat box** at the bottom to say what to change ("bigger sword", "different background"). Blender is downloaded **once** on first use (~383 MB), then everything runs offline on your PC. Under **Content → Thumbnails → "3D render"**.
- **Thumbnails: the skin actually shows now + real AI backgrounds** – two big things: (1) The **skin wasn't showing at all** because the render service we used was dead – now your skin comes in via a **working 3D renderer** (NMSR) (set your Minecraft name in the "Channel" tab). (2) Instead of a flat gradient, Julia now generates a **real AI background** from your description (free image AI, no key) – your skin (with outline/shadow/glow) and the bold text go on top. Quality over speed: the AI backgrounds may take a minute or two. (Grounded in current high-CTR rules: one subject, rule of thirds, 2–3 punchy colors, separate the subject from the background.)
- **Thumbnails now look genuinely good (the skin pops)** – the thumbnail builder is now pro-level: your **Minecraft skin** goes in as a **cinematic 3D pose** (from the skin-render service, matching the category – e.g. "ultimate" for Minecraft gaming) and gets a **white sticker outline, soft drop shadow and accent glow** so it stands out from the background (instead of being slapped on flat). Plus a punchier background (spotlight + bokeh depth + vignette) and bigger, bold text with a thick outline and shadow. (A real Blender render isn't possible inside the app – but the poses are already real 3D renders of your skin.)
- **Thumbnail "from description" now works in Claude subscription mode too** – in Claude Code mode you got "Concept failed: a one-shot call isn't available in Claude Code mode". Now building the concept (and other one-shot calls) works via your **Claude subscription** too. The "from video" watching still needs a provider with image support – if your mode can't do images, Julia now says so clearly and points you to "from description".
- **Content Studio: real mini-app design + thumbnails from a description** – the Content module now looks like its **own app**: a studio header with channel image/badge and icon tabs (✂️ Editing · 🗓️ Planning · 🖼️ Thumbnails · ⚙️ Channel). New for **thumbnails**: choose between **"From video"** (Julia watches the video) and **"From description"** – just describe your thumbnail and Julia builds a **cinematic image with a real background** (gradient + ray burst + spotlight behind your skin + vignette) and bold text. And it's now **clearly visible** that you should pick **Gaming or Reaction** up top (sets the skin pose and style).
- **Content is now an "app inside the app" with 3 tabs + a channel bar** – rebuilt exactly as you asked: a **channel/account bar** on top (image, channel name, category badge – switch channels like on YouTube), then **three tabs**: **Editing** (editing jobs: pick a rough cut + say how/when to cut – an ordered queue), **Planning** (what you make when, with an idea and a short script) and **Thumbnails**. For thumbnails Julia **actually watches the video** (several stills go to the vision model), suggests the best still + text/colors/layout, fetches your **skin** from your **Minecraft name** (cinematic render pose) and builds a finished **1280×720 thumbnail** you can save. Each channel can have a **fixed category** (e.g. MoinMornhart → Minecraft gaming); otherwise you pick reaction/gaming per video. (Real YouTube login with live stats is the next step.)
- **Content: thumbnails + "analyze channel" now visibly works** – two fixes: (1) In the analysis area the **"Channel"** button now actually switches – the video upload disappears and the text field clearly asks for your **channel info** (before, nothing visibly happened). (2) There's a new **Thumbnails** section: pick a video and Julia grabs several good **stills (1280×720)** as a starting point – save any as an image. (Text/logo/your skin on top come next.)
- **Update: no more half-done state if the installer won't start** – if the auto-update installer failed to launch (a file was locked or a process held it), Julia used to **quit anyway** – the update never ran and the log just said "started" with no reason. Now Julia catches a failed installer start, **stays open** (instead of quitting into a half update) and writes the **exact cause** to the log. (The final "file locked" fix still needs your concrete log line; the permanent solution is a signing certificate.)
- **Content: "Your YouTube channel" is now collapsible** – the channel card in the Content tab is no longer shown permanently. It's now **collapsed** and only there for **setup**: expand it once, fill in link/name/language/folder, done – after that the area stays tidy.
- **Content module: cut engine (groundwork)** – the foundation for automatic editing is here and tested: from a video Julia detects silence and builds a **rough cut** (jump-cut, no Adobe), makes **vertical clips for TikTok/YouTube Shorts** (9:16) from it, and grabs **thumbnail stills** – all via FFmpeg. The content profile now also remembers your **Minecraft name** and a **thumbnail theme** (e.g. gaming/Minecraft, reactions). The UI for it (sections "Editing & planning" / "Thumbnails") comes next.
- **Content module: clean new design + video upload** – the "Content" tab now looks like the rest of Julia (same colors/cards/buttons, orange accents) instead of thrown together. New: under "Analyze" you can now **upload a video directly** – Julia transcribes it locally (Whisper) and then gives you the creator feedback. The channel link stays optional; you pick folders via a dialog; Julia makes the assets herself.
- **Minecraft: no longer stuck in water** – if Julia was wedged in shallow water with an active task, she sometimes couldn't get out. Now she surfaces on her own in that case and finds a new way out.
- **"Hey Julia" is recognized more reliably** – the wake-word recognition threshold was set too strictly and often swallowed "Hey Julia"; it's more sensitive now. Reminder: the wake word is **off by default** (turn on "wake word" in the tray/settings) and needs the **German Windows speech recognition** – if it still doesn't hear you, Julia now tells you more precisely why.
- **Content: have a video or channel analyzed** – the simple version is here: in the "Content" tab you paste your **video's transcript** (or your **channel's** info), and Julia gives **creator feedback like a YouTube coach** – hook, pacing/retention, structure, 3 title ideas, 3 thumbnail ideas and the most important improvements. Purely text-based (only text goes to the AI), honest and concrete feedback instead of platitudes.
- **Update failures are now traceable** – when an update failed, the log often showed **no reason**. Now Julia writes the **exact cause** to `update.log` (e.g. which file was locked and why, `EBUSY`/`EPERM`) – so a stubborn case can be fixed specifically. There's also a short **troubleshooting guide** (docs/update-fehlerbehebung.md): where the logs are, how to add an antivirus exclusion, and why a proper **signing certificate** is the real permanent fix.
- **Content module: edit-plan engine & rough cut (groundwork)** – the next building blocks for automatic video creation are in place: from transcript + creator profile + style profile Julia builds the **edit plan** (hook, chapters, clips with reasons – only text goes to the AI) and can, as a fallback without Adobe, assemble a **real rough cut via FFmpeg**. Not visible in the menu yet (comes with the analysis pipeline), but tested and ready as the foundation.
- **New "Content" module (first stage)** – a dedicated area where Julia will later turn raw footage into a finished cut **locally in the Adobe apps** (Premiere, After Effects, Photoshop). The first stage starts now: turn the module on in the "Content" tab and create your **creator profile** (channel name, colors/fonts, music/SFX folders, fixed rules, style profile) – it then applies to every project automatically. Profiles can be created, set active, deleted and im-/exported as JSON. (Off by default; editing/thumbnails/watch mode come in the next stages.)
- **Updates: more robust when a "file is locked"** – if an update failed because an antivirus or backup tool briefly locked a file, Julia used to give up immediately. Now she automatically **retries** the affected steps a few times with a short pause (checkout, dependencies, installer download – the checksum is still verified every time). If that doesn't help, it's usually **security software** locking the folder (there's a Defender-exclusion button in the settings for that).
...
- **Minecraft: smarter defense** – Julia now defends herself with some sense: if she has **no proper weapon or low health**, she no longer charges blindly into melee but **keeps her distance and dodges** – yet fights back if the attacker corners her. With a weapon and enough health she fights back normally. And in the **social BETA mode** she also reacts **verbally** (briefly warns the attacker in the in-game chat) instead of fighting silently.
- **Minecraft: no more chat spam + Julia defends herself** – two wishes: (1) While playing through, Julia wrote her crafting/status messages ("crafted 48× oak_planks", "crafting table is at …") into the **in-game chat**. That's now an **on/off toggle** (Minecraft area → "Write progress into the in-game chat"), **off by default** – you still see the messages in the Julia window, but the game chat stays quiet; she can of course still reply normally. (2) **Julia now defends herself against players** who attack her: if someone hits her, she fights back for a while (she never attacks her own player). She already defended against monsters.
- **Won't start after an update? New emergency stage against third-party software** – if Julia crashed right after an update (`-2147483645`) and even the previous graphics stages no longer came up, it usually wasn't the graphics at all, but a **third-party DLL** (antivirus, game overlay, tuning tool) hooking into Julia, or Windows code-integrity. New: a **final emergency stage** that targets exactly that (sandbox off + renderer code-integrity check off) – the documented Windows fix for this crash. Julia tries it automatically before giving up. If the picture still doesn't come up, it says clearly: update the graphics driver or close the overlay/tuning tool.
- **Black window? Now recoverable without reinstalling** – on PCs with a broken graphics driver or intrusive security software the UI could stay black even in the fallback stage, and the only way back in was a reinstall. The emergency dialog now offers two buttons: **"Reset graphics & restart"** (runs the graphics rescue stages from the top again – no reinstall needed) and **"Update graphics driver"** (opens your vendor's driver page). If it's still black afterwards, it's almost certainly the graphics driver or security software hooking into Julia. Also: the Defender exclusion no longer wrongly reports "UAC declined" when you clicked "Yes" – if **Tamper Protection** blocks the change, Julia now explains exactly how to add the exclusion by hand.
- **Julia now operates real editors (Premiere, DaVinci, CapCut, After Effects …)** – you can hand her the PC: Julia opens the editor and operates it herself on screen (import, cut, **keyframe animation** in After Effects, export) – with the right knowledge of shortcuts and workflow for Premiere Pro, DaVinci Resolve, CapCut, After Effects, VEGAS, Shotcut, Filmora and the **Adobe Creative Cloud** (incl. Dynamic Link & Media Encoder). The old built-in **"Cut" tab has been removed** – it's replaced by the real programs (quick ffmpeg cuts still work; ffmpeg is fetched on demand).
- **Graphics fallback actually renders now (instead of crashing the renderer)** – the previous fallback disabled the problematic GPU process but left the UI without any drawing path, so it crashed. Now that stage does all graphics **in the main process in software** (no separate GPU process) – so the picture comes up even on PCs with a broken graphics driver. (The update itself works; if the UI still stays black afterwards, an up-to-date graphics driver helps – link in Settings → System.)
- **Defender exclusion: clear feedback instead of a cryptic error** – the "Exclude from Windows Defender" button showed a raw PowerShell error ("access denied") on problems. Julia now waits for the admin step and **verifies the exclusion was actually set**. If the Windows prompt (UAC) was declined, it says so plainly ("please click Yes"); if **Tamper Protection** blocks the automatic change, it explains step by step how to add the exclusion by hand in Windows Security.
