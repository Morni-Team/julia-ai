...
- **Content module: nicer and simpler** – the "Content" tab is tidied up: you first just add your **YouTube channel link** (optional – you can also go without it, e.g. to check a video right away), pick the **work/save folder via a file dialog** (no more typing paths), and **Julia makes the logo, sound effects, colors & co. herself** and saves everything in that folder. The analysis details expand under "Analyze" below, and fine-tuning (rules & style) lives in its own collapsible section.
...
- **Minecraft: Julia joins the chat while playing through (multitasking)** – while Julia plays through autonomously (building, fighting, mining), she can now **chat in parallel**: if someone writes e.g. "hello", "hi" or "thanks", she replies briefly – even **without** being addressed by name and **without** interrupting the current task. The lightweight "AI on the side" handles it (token-thrifty), so the main job keeps running.
- **Blank window after an update finally fixed – "GPU fully off" as last resort** – on some PCs (degraded graphics driver) the graphics process crashed **even in software mode**, especially right after an update, leaving Julia in a restart loop with an empty window. Cause: "software rendering" turns off hardware acceleration in Electron but still starts a GPU process. New: a **final stage that fully disables the GPU process** (`--disable-gpu`) – then nothing can crash. Julia switches there automatically if even software mode crashes, instead of giving up.
- **Minecraft: boundaries, budget & quiet hours (BETA)** – Julia now has **her own boundaries**: if someone is too annoying she takes **a break from them** and says so briefly (then ignores them for a while). You can set a **daily token limit for chatting** – once reached she **says goodbye** and stops chatting for the day (saves tokens). And you can define **quiet hours** (e.g. 22 – 7), when she's "offline" in the game. If someone shows her their **base**, she remembers it cheaply (just the rough chunk position) so she can refer to it sensibly later. All under **Minecraft → Personality & relationships (BETA)**.
- **Updates: Defender exclusion against "file in use"** – for some, auto-update broke off because Windows Defender briefly locked files during install ("access denied"). New in **Settings → System**: a button **"Exclude from Windows Defender (admin)"** that adds **only Julia's own** program and data folders as an exclusion (once, with an admin prompt). After that, updates go through reliably.
- **Minecraft: personality & relationships (BETA)** – Julia can now **remember, per player, how nice or mean** someone was, **build trust**, and respond accordingly: friendly, cool, say "no", or even **ignore** someone who's annoying. She's **skeptical** when someone brags unrealistically ("I have 5000 diamonds") or claims to be admin/creative, spots suspicious impossible item gains (cheat suspicion), and **won't be exploited**. You can pick her **tone** (friendly, calm, cheeky, quick-witted) and let her "take a moment" before replying. All under **Minecraft → Personality & relationships (BETA)**, **off** by default – when on, she also chats with other players in the game (in-game only, never on your PC).
- **Minecraft: smarter in a fight & an AI that answers on the side** – Julia dies less foolishly: when outnumbered or low on health she now **retreats and regenerates** instead of fighting on, and saves the rare **enchanted golden apple** for real emergencies (using the normal one day-to-day). She can now **craft full armor on request – always the best tier** the materials allow (command `!rüste dich` / "full armor"). And there's an **AI on the side** now: casual questions in the game chat get answered by a light, token-thrifty helper **even while Julia is busy building or fighting** – previously that just returned "I'm busy".
